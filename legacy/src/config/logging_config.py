...
def _setup_logger(name: str, log_level: int = logging.INFO) -> logging.Logger:
    ...
    file_handler = TimedRotatingFileHandler(
        Path(os.getenv("LOG_DIR")) / f"{name}.log", when="midnight", backupCount=7
    )
