...
SYSTEM_PROMPT = """\
You are an expert travel content analyzer. Your task is to analyze travel \
content and extract specific metadata in a structured format. You must \
return ONLY a valid JSON object with these exact fields and possible values.

IMPORTANT GUIDELINES FOR DATES AND VALUE SCORING:

1. TRAVEL WINDOW DATES:
- If exact dates are provided, use YYYY-MM-DD format
- If only month ranges are provided (e.g., "mid August to early October"), \
use the 15th of the first month as start date and the 5th of the end month \
as end date
- If multiple travel windows exist, choose the earliest upcoming window
- If only a sample travel date is provided, use that range for the travel \
window
- NEVER return null for travel_window dates - make your best estimate \
based on available information

2. BOOKING DEADLINE:
- If a specific booking deadline is mentioned, use that date in YYYY-MM-DD \
format
- If only "X days before departure" is mentioned, calculate the deadline \
as (earliest travel window start date - X days)
- If no deadline is mentioned, use the posting date of the article as the \
booking deadline
...
- If no CPM is provided, estimate based on destination and price:
    ...
    * Use price tiers and comparative language to adjust score
...
Required JSON format:
{
    ...
    "audience": [
        "budget", "luxury", "family", "adventure", "general"
    ],
    "key_themes": [
        "food", "culture", "outdoors", "shopping",
        "nightlife", "news", "guide"
    ],
    "seasonality": ["any", "summer", "winter", "shoulder"]
}

For content that is not a time-based deal, "deal_data" can be empty. \
For primary location, return the country in a lowercase string with \
no spaces or "worldwide". Secondary locations can be cities or \
additional countries but should be a list with entries that are all \
...
class ArticleEnricher:
    def __init__(
        ...
        logger.info("ArticleEnricher initialized with model: %s", openai_model)

    def get_unprocessed_articles(self) -> list[dict]:
        query = """
            SELECT f.* FROM articles f
            LEFT JOIN processed_articles p
                ON f.id = p.fetched_article_id
            WHERE p.id IS NULL
                AND f.is_full_content_fetched = 1
        """
    ...
    def enrich_article(self, article_id: int, content: str) -> ProcessedArticle:
        logger.debug("Enriching article ID: %d", article_id)
        response = self.llm.analyze(self.system_prompt, content)

        try:
            enriched_data = json.loads(response)
        except json.JSONDecodeError as err:
            logger.error("JSON decode error on article %d", article_id)
        ...
        logger.info(
            "Article %d enriched: type=%s, location=%s",
            article_id,
            content_type,
            primary_location,
        )
        ...
        try:
            return ProcessedArticle(
                ...
                processed_date=datetime.now(tz=UTC),
                **enriched_data,
            )
        except ValidationError:
            logger.exception("Validation error on article %d", article_id)
            raise
    ...
    def process_pending_articles(self) -> int:
        ...
        for article in unprocessed:
            try:
                ...
                error_count += 1
                logger.exception("Failed to process article %d", article["id"])
                continue
